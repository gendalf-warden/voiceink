...
- [x] 27 юнит-тестов (KeyMap, AppState, Config, StringExtensions) + pre-merge валидация
...
5. **История диктовок**: последние 10 в меню с копированием

### Фаза 5 — Качество транскрипции
6. **Режимы пост-обработки** (переключение в меню или горячей клавишей):
...
7. **Режим перевода**: говоришь на одном языке → текст на другом
8. **Smoke-тесты транскрипции**: эталонные WAV + проверка спецсимволов, чисел, англицизмов

### Фаза 6 — Продвинутые фичи
9. **Голосовые команды**: "новый абзац", "точка", "удали последнее слово"
10. **Непрерывная диктовка**: toggle-режим без удержания клавиши для длинных текстов
11. **Протестировать LLM модели**: Qwen 3.5 разные размеры
12. **Статистика диктовок**: счётчик слов за день/неделю/месяц

### Фаза 7 — Auto-updater (когда будут пользователи)
13. **Разделение на компоненты**: бинарник, dylibs, модели — каждый со своей версией
14. **Первая установка**: скачивает все компоненты (~3.5 GB) с прогрессом
15. **Auto-updater**: проверяет `latest.json`, скачивает только изменённое
16. **Авто-обновление при запуске**: скачать, заменить, перезапустить, прогресс в splash
17. **Fallback**: если обновление не удалось — работает на текущей версии
