...
6. **Словарь замен**: пользовательская библиотека «неправильное → правильное» (напр. "Демале" → "ДеМоле"). Применяется после Whisper, до LLM. Редактирование через настройки, хранение в config
...
10. **Продвинутая транскрипция файлов** (план в 4 этапа):
   - *A. Chunked transcription* — разбивать WAV на 30с чанки (разрез по тишине ±5с), транскрибировать последовательно, LLM по чанкам (не на весь текст), стриминг результата в окно
   - *B. Улучшенное окно результата* — прогресс-бар N/total, текст появляется по мере готовности, редактируемый, Save as TXT/SRT/MD, переключатель таймстампов, статистика
   - *C. Batch & drag-drop* — multiple file selection, drag файлов на иконку в menu bar, очередь с общим прогрессом
   - *D. Экспорт* — TXT/SRT/MD, автосохранение рядом с исходником (video.mp4 → video.srt)
...
12. **Протестировать LLM модели**: Qwen 3.5 разные размеры
