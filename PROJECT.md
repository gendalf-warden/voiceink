...
19. **Диаризация спикеров (speaker labels) + редактирование меток**: при транскрипции определять кто говорит, размечать сегменты метками спикеров («Спикер 1», «Спикер 2»…) и дать UI для переименования/правки меток (например «Спикер 1» → «Анна»). Применимо в первую очередь к **файловой транскрипции** (встречи, интервью, подкасты) — диктовка обычно одноголосая. Три части:
    - **Детекция**: whisper.cpp сам не делает диаризацию — нужен отдельный шаг (VAD → speaker embeddings → кластеризация), результат сливается с whisper-сегментами по таймштампам. Кандидаты реализации: **FluidAudio** (Swift-нативная диаризация, см. п.16 — оценить вместе) или pyannote-подобный пайплайн. Учесть лицензию (тема коммерческого блокера, ср. п.12) и размер/RAM модели эмбеддингов.
    - **Разметка**: расширить `FileTranscriptionManager` пайплайн — сегменты с `speaker` полем; формат вывода с префиксами спикеров (как в SRT/диалоге).
    - **Редактирование меток**: UI в `TranscriptionResultWindowController` — список обнаруженных спикеров с инлайн-переименованием, замена применяется ко всем сегментам спикера. Опционально — переназначение отдельного сегмента другому спикеру.
    Зависит от per-segment таймштампов (whisper verbose_json уже их даёт). ~оценить после R&D FluidAudio (п.16)
