...
|-----------|-----------|--------|
| Приложение | Swift 5.9, macOS 13+ | SPM, 18 файлов, ~600 KB binary |
...
### Фаза 2
1. Дистрибуция и обновления:
   - ~~**2a. .dmg инсталлер**: drag-to-Applications~~ ✅
   - ~~**2b. First-run wizard**: запрос Microphone + Accessibility с пояснениями~~ ✅
   - **2c. Notarization**: Apple Developer ID + notarize
   - **2d. Хостинг релизов**: GitHub Releases, манифест `latest.json` с SHA256
   - **2e. Разделение на компоненты**: бинарник, dylibs, модели — каждый со своей версией
   - **2f. Первая установка**: скачивает все компоненты (~3.5 GB) с прогрессом
   - **2g. Auto-updater**: проверяет `latest.json`, скачивает только изменённое
   - **2h. Авто-обновление при запуске**: скачать, заменить, перезапустить, прогресс в splash
   - **2i. Fallback**: если обновление не удалось — работает на текущей версии
   - ~~**2j. Dev/Prod разделение**: dev локальная, деплой только с подтверждением~~ ✅
2. История диктовок (последние 10) — в меню с копированием
3. Процесс разработки: git-flow, ветвление, тестирование, CI, код-ревью, релизный процесс
4. Режим перевода: диктовка на одном языке → текст на другом
5. Smoke-тесты транскрипции: эталонные WAV + проверка спецсимволов, чисел, англицизмов
6. Протестировать другие LLM модели (Qwen 3.5 разные размеры)
7. Статистика диктовок: счётчик слов за день/неделю/месяц, пункт в меню
